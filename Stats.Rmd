...
In our everyday life we are bombarded with all kinds of infomation. Naturally, some of this information comes from legitimate sources (e.g. scientific articles or news agencies) and some of it isoverheard information from a person. Eventhough news worthy information is often fit for provoking anxiety, we don't know if it would be less or more anxiety evoking if we heard the information from someone we know instead.
...
For this experiment, we took the state (current) anxiety levels of participants as the response variable. Furthermore, we controlled for subjective levels of perceived everyday stress, depressive thougths, suicidal ideation and current relaxed state, in a manner to be appointed from 1 to 10 in a likert scale. In addition to that, we asked for demographic informations of sex, age, type of the governance in the district they lived in the most (e.g. city, town, etc.), their income level, drug or alcholol addiction, any medical or psychological condiiton that affects their thinking processes, and the level of information they had about nuclear power plants.  
...
Experiment <- Experiment %>% 
  rename(Group= Grup, Anxiety = Toplam ,  Sex = Cinsiyet,  Age =Yaş ,  City= Enuzunşehir , Income=  GelirSeviye, 
          Daily.Stress =Günlükstres , Alcholol.Add=  Alkol , Depressive.Thoughts=  Depresifdüşünce , Suicidal.Ideation= İntihar ,
          Medical.Cond= TıbbiSorun ,  Psycho.Cond=PsikoSorun ,  Drug.Add=İlaç ,  Relaxed.AtM=ŞuanRahat , N.Power.Know= SantralBilgi)
...
Let's get a bit fancy and plot boxplot's with ANOVA results 
